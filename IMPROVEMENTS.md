...
- ✅ 5. Elephant-foot relief — auto perimeter trim when foot is sharp; chamfer/fillet still recommended
...
- ✅ 20. Per-edge selection (round/chamfer only chosen edges) — long/short sides
- ✅ 21. Variable / asymmetric base edge — bias slider
...
- ✅ 47. Validation/status panel — manifold, min-wall, material, metrics, warp/feature warnings
...
### Wave 4 — Remaining 34 items ✅
- **Print/view:** layer-step preview, 3MF/STEP export, tiled bed split export, tolerance strip.
- **Edges:** decouple vertical corners, long/short edge sides, bias, chamfer angle, bead profile, SVG cross-section, cove-under-brim feedback.
- **Features:** gasket groove, insert bosses, dividers, stack lip, nest taper, finger scoop, handles, label slot, vents, keyhole mount, Gridfinity underside.
- **Viewport:** wall heatmap, section clip, bbox dimensions, turntable, filament preview, lid press animation.
- **UX:** undo/redo (⌘Z/⇧⌘Z), mm/in toggle, shareable URLs, design-intent chips, param tooltips, arrow-key nudge on sliders.

### Wave 3 — Inspection & plate layout ✅ (#3, 4, 10, 41)
- **#3:** `overhangAnalysis.ts` — face slope vs 45° threshold; stats in Output panel; viewport heatmap toggle colors body/lid by severity.
- **#4:** Auto underside chamfer on mounting flange (scaled frustum subtract at rim) to reduce brim overhang.
- **#10:** `printPlate.ts` bed-fit card in Print Setup (220/256/300 mm presets, body+lid side-by-side hint).
- **#41:** Lid seated toggle — ghost overlay on the rim vs exploded gap; preview controls under the view cube.

### Wave 2.5 — Draft / surfacing fixes ✅ (#5)
- **Draft:** interior cavity warp now measures taper from the cavity floor (`profileBaseZ`), not global Z — fixes floor–wall spikes and the “whole box grew” look.
- **Surfacing:** restored honeycomb/cells/knurl field behavior; hex blends discrete nubs at high sharpness.
- **Stats:** nominal L×W×H vs outer bbox; wall-draft hint clarifies floor footprint.
- **#5:** automatic elephant-foot perimeter trim when underside is sharp (~0.45× layer height).

### Wave 2 — Base & Floor ✅ (#25, 27, 34)
